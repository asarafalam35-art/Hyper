...
app.get('/api/songs/trending',async(req,res)=>{try{
  ...
  const results=await Promise.all(terms.map(async term=>{
    ...
    const r=await fetch(u); if(!r.ok) return []; const j=await r.json(); return (j.results||[]).map(x=>({
      ...
      artworkUrl100:x.artworkUrl100||'',previewUrl:x.previewUrl||'',genre:x.primaryGenreName||'',
      language:term
    }));
  }));
...
app.get('/api/trending/mixed',async(req,res)=>{try{
  const [songsR,newsR]=await Promise.all([fetch('http://127.0.0.1:'+PORT+'/api/songs/trending').catch(()=>null),fetch('https://news.google.com/rss?hl=hi&gl=IN&ceid=IN:hi').catch(()=>null)]);
  ...
  const mixed=[]; const max=Math.max(songs.length,news.length,reels.length,shorts.length);
  for(let i=0;i<max;i++){if(songs[i])mixed.push({kind:'song',item:songs[i]});if(reels[i])mixed.push({kind:'reel',item:reels[i]});if(news[i])mixed.push({kind:'news',item:news[i]});if(shorts[i])mixed.push({kind:'short',item:shorts[i]});}
  res.json({items:mixed.slice(0,120),songs:songs.slice(0,150),news:news.slice(0,30),reels,shorts});
