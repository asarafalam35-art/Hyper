...
function seed(){
  ...
  const adminEmail=String(process.env.ADMIN_EMAIL||'asarafalamt20@gmail.com').toLowerCase();
  const adminPassword='A2aryann';
  users.push({id:crypto.randomUUID(),username:'admin',email:adminEmail,name:'Hyper Admin',bio:'Hyper administrator',avatar:'A',password:bcrypt.hashSync(adminPassword,10),privateProfile:false,notifications:true,followers:[],following:[]});
...
function loginSession(req,res,u){const sid=crypto.randomUUID();sessions.set(sid,u.id);res.cookie('hyper_sid',sid,{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',path:'/',maxAge:1000*60*60*24*30})}
app.post('/api/login',(req,res)=>{const email=String(req.body?.email||'').trim().toLowerCase();const password=String(req.body?.password||'');if(!email||!password)return res.status(400).json({error:'Email and password are required.'});const u=users.find(x=>String(x.email).toLowerCase()===email);if(!u||!bcrypt.compareSync(password,u.password))return res.status(401).json({error:'Invalid email or password.'});loginSession(req,res,u);res.json({user:safeUser(u)})});
...
app.get('/api/conversations',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const map=new Map();for(const m of messages){if(m.from!==req.user.id&&m.to!==req.user.id)continue;const otherId=m.from===req.user.id?m.to:m.from;const other=users.find(u=>u.id===otherId);if(!other)continue;const old=map.get(otherId);if(!old||old.lastMessage.createdAt<m.createdAt)map.set(otherId,{user:safeUser(other),lastMessage:m})}res.json({conversations:[...map.values()].sort((a,b)=>b.lastMessage.createdAt-a.lastMessage.createdAt)})});
app.post('/api/calls',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const to=users.find(u=>u.id===req.body?.to);if(!to)return res.status(404).json({error:'User not found.'});const type=req.body?.type==='video'?'video':'audio';const call={id:crypto.randomUUID(),from:req.user.id,to:to.id,type,offer:req.body?.offer,answer:null,createdAt:Date.now()};calls=calls.filter(c=>c.to!==req.user.id&&c.from!==req.user.id);calls.push(call);res.json({call:{id:call.id,type:call.type}})});
app.get('/api/calls/incoming',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const c=calls.find(x=>x.to===req.user.id&&!x.answer);if(!c)return res.json({call:null});const u=users.find(x=>x.id===c.from);res.json({call:{id:c.id,type:c.type,offer:c.offer,fromUser:safeUser(u)}})});
app.get('/api/calls/:id',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const c=calls.find(x=>x.id===req.params.id&&(x.from===req.user.id||x.to===req.user.id));if(!c)return res.status(404).json({error:'Call not found.'});res.json({call:{id:c.id,type:c.type,answer:c.answer,status:c.answer?'answered':'ringing'}})});
app.post('/api/calls/:id/answer',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const c=calls.find(x=>x.id===req.params.id&&x.to===req.user.id);if(!c)return res.status(404).json({error:'Call not found.'});c.answer=req.body?.answer;res.json({ok:true})});
app.post('/api/calls/:id/end',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});calls=calls.filter(c=>c.id!==req.params.id);res.json({ok:true})});
...
app.post('/api/messages/share',(req,res)=>{if(!req.user)return res.status(401).json({error:'Login required.'});const to=users.find(u=>u.id===req.body?.to);const post=posts.find(p=>p.id===req.body?.postId);if(!to||!post)return res.status(404).json({error:'User or post/reel not found.'});if(to.id===req.user.id)return res.status(400).json({error:'You cannot send this to yourself.'});const m={id:crypto.randomUUID(),from:req.user.id,to:to.id,text:'',sharedPostId:post.id,createdAt:Date.now()};messages.push(m);res.json({message:m})});
